...
#
    mj_model = mujoco.MjModel.from_xml_path(
        (HUMANOID_ROOT_PATH +'/'+'humanoid.xml'))
  ...
  def reset(self, rng: jp.ndarray) -> State:
    ...
    obs = self._get_obs(data, jp.zeros(self.sys.nu ), jp.zeros( self.cloth_x_dim))
  ...
  def step(self, state: State, action: jp.ndarray) -> State:
    ...
    #cloth_step here
    cloth_x = state.obs[-3:]
    newcloth_x = cloth_x + 0.1 * jp.ones(self.cloth_x_dim)
    print('step ... ')
  ...
  def _get_obs(
      self, data: mjx.Data, action: jp.ndarray, cloth_x
  ) -> jp.ndarray:
...
if train:
    ...
    def progress(num_steps, metrics):
      ...
      r=metrics['eval/episode_reward']
      r_std=metrics['eval/episode_reward_std']
      y_data.append(r)
      cloth_x.append(metrics['eval/episode_cloth_x'])
      ydataerr.append(r_std)
    ...
    fig = plt.figure()
    ax = fig.subplots(2,1)
    ax[0].set_xlim([0, train_fn.keywords['num_timesteps'] * 1.25])
    ax[0].set_ylim([min_y, max_y])

    ax[0].set_xlabel('# environment steps')
    ax[0].set_ylabel('reward per episode')
    ax[0].set_title(f'y={y_data[-1]:.3f}')

    ax[0].errorbar(x_data, y_data, yerr=ydataerr)
    ax[1].plot(x_data, cloth_x, '-o')
    fig.savefig(os.path.join(TRAINED_MODEL_PATH,'progress.png'))
...
dummy_train_fn = functools.partial(
    ...
    unroll_length=1, num_minibatches=1, num_updates_per_batch=8,
    discounting=0.97, learning_rate=3e-4, entropy_cost=1e-3, num_envs=1,
    batch_size=1, seed=0)
